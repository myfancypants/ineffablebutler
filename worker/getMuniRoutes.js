...
var getRouteStops = function(routeId, runsArray) {
  ...
  for(var i = 0; i < runsArray.length; i++) {
    ...
    queue.push(runsArray[i]);

    getRequest("http://proximobus.appspot.com/agencies/sf-muni/routes/" + routeId + "/runs/" + runsArray[i] + "/stops", "", function(error, response, body) {
        ...
        var insertStops = function(i) {
         ...
         if(i === pathStop.length)
          ...
          NEO_db.cypherQuery(
            ...
            ' CREATE (stopOne)-[ r: `' + routeId + '_' + runId + '` ]->(stopTwo)' +
